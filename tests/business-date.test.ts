...
test("Eastern business calendar date defaults a calendar control to today", () => {
  const beforeEasternMidnight = easternBusinessCalendarDate(new Date("2025-01-02T04:59:59.000Z"));
  const afterEasternMidnight = easternBusinessCalendarDate(new Date("2025-01-02T05:00:00.000Z"));

  assert.equal(localCalendarYmd(beforeEasternMidnight), "2025-01-01");
  assert.equal(localCalendarYmd(afterEasternMidnight), "2025-01-02");
  assert.equal(beforeEasternMidnight.getHours(), 0);
  assert.equal(afterEasternMidnight.getHours(), 0);
});
