...
- **Templates** (`/app/admin/email/templates`, backed by `/api/email-templates` GET/POST/PATCH/DELETE, role-gated + org-isolated, mirrors the email-settings pattern): each template has a `templateType` of `email` or `text`. Email templates require a subject; text templates hide it. The editor supports clickable merge-variable insertion (e.g. `{{first_name}}`, `{{balance}}`), preview, and confirmed deletes. A per-template **Send** action opens a dialog to pick a type-compatible active integration, search/multi-select accounts, and post to `/api/campaigns/send` with the `templateId`. The server resolves the template (org-scoped), enforces channel compatibility (email template → email integration, text → sms), and forwards the template content (`subject`/`body`) plus accounts to the Chain `/campaigns/send` endpoint. Contact values are derived by integration type: email uses `debtor.email` (falling back to debtor contacts), SMS pulls a phone from `/api/debtors/:id/contacts` (primary first).
...
### Caching Strategy
- **Service Worker** (`client/public/sw.js`): Network-first for navigations and non-hashed assets, cache-first for Vite hashed assets (`/assets/*`). App shell pre-cached on install. Old caches purged on SW activation. `CACHE_NAME` carries a `__SW_VERSION__` placeholder; the worker does NOT call `skipWaiting()` on install — a new worker waits (so no surprise mid-task reloads) and auto-activates when the app is fully relaunched. A `SKIP_WAITING` message handler lets the page activate a waiting worker on demand.
- **Per-deploy SW versioning** (`server/static.ts`): A `GET /sw.js` route (registered before the static middleware) injects a build version into the worker — `sha1` of the built `index.html` (12 chars), which changes whenever hashed asset filenames change, i.e. every deploy. This guarantees each deploy ships a byte-different `sw.js` so the browser reliably detects the update. In dev the literal placeholder remains.
- **SW Registration & update prompt** (`client/src/lib/pwa-update.ts`, registered from `client/src/main.tsx`): registration moved out of `index.html` into the app. Uses `updateViaCache: 'none'`. On detecting a waiting/new worker (with an existing controller), dispatches `pwa-update-available`; `<UpdatePrompt>` (`client/src/components/update-prompt.tsx`, mounted in `App.tsx`) shows a "new version available — Reload to update" banner. `applyPwaUpdate()` posts `SKIP_WAITING` and reloads once on `controllerchange` (single-reload guard + 3s fallback). `forcePwaRefresh()` clears caches, unregisters workers, and reloads — surfaced via `<AppRefreshHelp>` (`client/src/components/app-refresh-help.tsx`) on the login and collector-install pages, alongside desktop uninstall/reinstall instructions. `index.html` retains only a one-time cleanup (key `sw-cleanup-v7`) that unregisters obsolete workers from a different script URL.
